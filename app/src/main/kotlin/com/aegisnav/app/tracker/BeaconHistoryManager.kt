...
class BeaconHistoryManager @Inject constructor(
    ...
    /** Tracks MACs for which a GATT identification has already been requested. */
    private val gattRequestedMacs: MutableSet<String> = ConcurrentHashMap.newKeySet()
    ...
    fun onBleSighting(
        ...
        scope.launch {
            try {
                ...
                // GATT threshold check — use >= to handle concurrent inserts or restarts
                val count = beaconSightingDao.countForMac(mac)
                if (count >= GATT_THRESHOLD_SIGHTINGS) {
                    if (mac in gattRequestedMacs) return@launch
                    gattRequestedMacs.add(mac)
