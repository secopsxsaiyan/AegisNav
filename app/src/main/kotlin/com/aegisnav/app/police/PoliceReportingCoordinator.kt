...
class PoliceReportingCoordinator @Inject constructor(
    ...
    // Sentry rate limiting: last time a police detection event was sent (ms)
    private var lastPoliceSentryMs = 0L
    ...
    private suspend fun handleSighting(sighting: PoliceSighting) {
        ...
        val policeSentryNow = System.currentTimeMillis()
        if (policeSentryNow - lastPoliceSentryMs >= 300_000L) {
            lastPoliceSentryMs = policeSentryNow
            crashReporter.captureMessage("Police detection: category=${sighting.detectionCategory} confidence=${sighting.confidence}")
        }
