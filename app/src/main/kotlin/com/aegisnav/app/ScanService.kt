...
class ScanService : Service() {
    ...
    /**
     * Reports scan health diagnostics to local log.
    ...
    private fun reportScanDiagnostics() {
        ...
        if (uptimeMin >= 5 && bleForegroundCallbackCount == 0L) {
            AppLog.w("ScanService", "BLE foreground scan zero results after ${uptimeMin}m")
        }
    }
                ...
                try {
                    ...
                    location?.let { loc ->
                        signalTriangulator.addObservation(
                            log.deviceAddress, loc.latitude, loc.longitude, distM
                        )
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (intent == null) {
            ...
            val userWants = kotlinx.coroutines.runBlocking(Dispatchers.IO) {
        ...
        if (Build.VERSION.SDK_INT >= 36) {
            userScanIntensity = kotlinx.coroutines.runBlocking(Dispatchers.IO) {
                SecureDataStore.get(this@ScanService, "an_prefs")
    ...
    private fun onConvoyAlert(alert: ConvoyDetector.ConvoyAlert) {
        scanScope.launch(scanErrorHandler) {
            val now = System.currentTimeMillis()
    ...
    private fun onCoordinatedSurveillanceAlert(alert: CoordinatedSurveillanceDetector.CoordinatedAlert) {
        scanScope.launch(scanErrorHandler) {
            val now = System.currentTimeMillis()
