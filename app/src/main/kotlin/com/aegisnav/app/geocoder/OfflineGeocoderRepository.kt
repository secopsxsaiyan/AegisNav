...
class OfflineGeocoderRepository @Inject constructor(
    ...
    /**
     * Sanitize a raw token or query string before embedding it in an FTS4 MATCH expression.
     *
     * FTS4 treats the following as special:
     *   - Punctuation operators: `"` (phrase), `*` (prefix), `(`, `)`, `+`, `-`
     *   - Boolean word operators: OR, AND, NOT (uppercase only by default, but strip any case)
     *   - Proximity operator: NEAR
     *   - Single quotes: break the surrounding SQL even inside a bound `?` value when
     *     re-quoted by some SQLite helpers; strip to be safe.
     *
     * After sanitization the token is safe to use as a plain prefix term (appended with `*`).
     */
    internal fun sanitizeFtsQuery(input: String): String {
        // 1. Remove FTS4 punctuation operators
        var s = input.replace(Regex("""["*()+\-]"""), "")
        // 2. Remove single quotes (SQL/FTS tokenizer risk)
        s = s.replace("'", "")
        // 3. Remove FTS4 boolean / proximity operators as whole words (any case)
        s = s.replace(Regex("""(?i)\b(OR|AND|NOT|NEAR)\b"""), " ")
        return s.trim()
    }
    ...
    private fun buildFtsTokens(tokens: List<String>, dropShort: Boolean): String {
        ...
        return filtered.joinToString(" ") { token ->
            val safe = sanitizeFtsQuery(token)
