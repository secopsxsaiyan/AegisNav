...
fun SettingsScreen(
    ...
    val selectedStates by settingsViewModel.selectedStates.collectAsStateWithLifecycle()
    val tileManifest by settingsViewModel.tileManifest.collectAsStateWithLifecycle()
    val installedStateCount by settingsViewModel.installedStateCount.collectAsStateWithLifecycle()
    ...
    if (showDataDownload) {
        Box(
            ...
            Box(modifier = Modifier.widthIn(max = 600.dp).fillMaxHeight()) {
                DataDownloadScreen(
                    onBack = {
                        settingsViewModel.refreshInstalledCount()
        ...
        Box(
            ...
            Box(modifier = Modifier.widthIn(max = 600.dp).fillMaxHeight()) {
                StateSelectionScreen(
                    onComplete = { abbrs ->
                        settingsViewModel.saveSelectedStatesAndResetPreload(abbrs)
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            // Hidden in release — online features not available until Phase 9.
            // Gated behind FEATURE_ONLINE_MODE_AVAILABLE to prevent user confusion.
            if (FeatureFlags.FEATURE_ONLINE_MODE_AVAILABLE) {
                ...
                SettingsCard(onClick = { showDataDownload = true }) {
                    Row(
                        ...
                        Column(modifier = Modifier.weight(1f)) {
                            ...
                            Text(
                                if (installedStateCount > 0) "$installedStateCount state${if (installedStateCount > 1) "s" else ""} downloaded • 78,000+ US cameras available"
                                else "No states downloaded • 78,000+ US cameras available",
                                style = MaterialTheme.typography.labelSmall,
                                color = if (installedStateCount > 0) Color(0xFF43A047) else Color.Gray
                            )
                        }
            ...
            item {
                val downloadedStates = remember(showDataDownload) {
                    com.aegisnav.app.data.DataDownloadManager.ALL_STATES.filter {
                        com.aegisnav.app.data.DataDownloadManager.isTilesInstalled(context, it.code)
                    }
                }
