...
fun DataDownloadScreen(
    ...
    LaunchedEffect(Unit) {
        val fromDataStore = DataDownloadManager.ALL_STATES
            ...
            .map { it.code }
            .toSet()
        // Filesystem re-scan: catches states whose files exist but DataStore was wiped
        val fromDisk = TileSourceResolver.downloadedStateCodes(context).toSet()
        installedStates = fromDataStore + fromDisk
        AppLog.i("DataDownloadScreen", "installedStates: fromDataStore=$fromDataStore fromDisk=$fromDisk merged=$installedStates")
...
private fun StateDownloadRow(
    ...
    // Observe WorkManager for this state's download job
    val workInfos by WorkManager.getInstance(context)
        .getWorkInfosForUniqueWorkLiveData(TileDownloadWorker.workName(state.code))
