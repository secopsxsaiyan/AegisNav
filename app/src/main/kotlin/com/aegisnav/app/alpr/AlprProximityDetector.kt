...
class AlprProximityDetector @Inject constructor(
    ...
    data class AlprCluster(
        ...
        fun centroid(): Pair<Double, Double> {
            ...
            signals.forEach { s ->
                val dist = AlprProximityDetector.rssiToDistance(s.rssi, s.scanType).coerceAtLeast(0.1)
                val w = 1.0 / (dist * dist)
        ...
        fun accuracyMeters(): Double {
            ...
            return signals.map { AlprProximityDetector.haversineMeters(cLat, cLon, it.lat, it.lon) }.average()
        }
    }
    ...
    fun onScanResult(log: ScanLog) {
        ...
        scope.launch {
            try {
                ...
                val closest = nearby.minByOrNull { AlprProximityDetector.haversineMeters(lat, lon, it.lat, it.lon) }
                    ?.takeIf { AlprProximityDetector.haversineMeters(lat, lon, it.lat, it.lon) <= ALPR_LOOKUP_RADIUS_M }
                    ?: return@launch
                ...
                if (cluster.signals.isNotEmpty()) {
                    ...
                    if (AlprProximityDetector.haversineMeters(lat, lon, cLat, cLon) > CLUSTER_RADIUS_M) return@launch
                }
    ...
    private suspend fun emitAlprProximity(cluster: AlprCluster) {
        ...
        val sightingsJson = JSONArray(cluster.signals.map { s ->
            JSONObject().apply {
                ...
                put("estimatedDistanceM", AlprProximityDetector.rssiToDistance(s.rssi, s.scanType))
            }
        })
