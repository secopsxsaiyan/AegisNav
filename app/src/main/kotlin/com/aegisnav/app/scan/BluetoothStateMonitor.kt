...
class BluetoothStateMonitor : BroadcastReceiver() {
    ...
    private fun handleBluetoothOff(context: Context) {
        val dataStore = SecureDataStore.get(context, PREFS_NAME)
        // TODO(phase-refactor): Convert to Flow-based async read
        dataStore.editBlocking { this[longPreferencesKey(PREFS_BT_DISABLED_AT)] = System.currentTimeMillis() }
        ...
        // TODO(phase-refactor): Convert to Flow-based async read
        val hasActiveAlert = dataStore.readBlocking()[booleanPreferencesKey("has_active_tracker_alert")] ?: false
        if (hasActiveAlert) {
    ...
    private fun handleBluetoothOn(context: Context) {
        ...
        val dataStore = SecureDataStore.get(context, PREFS_NAME)
        // TODO(phase-refactor): Convert to Flow-based async read
        val snapshot = dataStore.readBlocking()
        ...
        dataStore.editBlocking { remove(longPreferencesKey(PREFS_BT_DISABLED_AT)) }
