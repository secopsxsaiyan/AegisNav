...
class CrossCorrelationEngine @Inject constructor(
    ...
    companion object {
        ...
        /** BLE/WiFi history entries older than this are eligible for eviction. */
        private const val HISTORY_TTL_MS = 60 * 60_000L

        /** Minimum interval between successive history-prune sweeps to avoid overhead. */
    ...
    /** Timestamp of the last history-prune sweep; 0L means never pruned. */
    ...
    /**
     * Evicts stale entries from [recentBle] and [recentWifi] using a 60-minute TTL.
     * This is a longer-horizon sweep complementing [pruneStale] (which uses a 10-min TTL
     * and runs every scan cycle). Throttled by [lastPruneMs] — call site checks interval.
     */
    private fun pruneStaleHistory() {
        val cutoff = System.currentTimeMillis() - HISTORY_TTL_MS
        recentBle.entries.removeAll { it.value.timestamp < cutoff }
        recentWifi.entries.removeAll { it.value.timestamp < cutoff }
        AppLog.d(TAG, "pruneStaleHistory complete")
