...
class FollowingDetector @Inject constructor(
    ...
    private suspend fun analyse(mac: String, sightings: List<ScanLog>): List<FollowingEvent> {
        ...
        for (i in uniqueZones.indices) {
            for (j in i + 1 until uniqueZones.size) {
                ...
                sentryCaptureThrottled("following_${finalEvent.mac.take(8)}", 300_000L,
                    "Following detected: mac=${finalEvent.mac.take(8)}XX:XX zones=2 distance=${finalEvent.distanceMeters.toInt()}m")
    ...
    private fun buildMultiLocationEvent(
        ...
        // B must be observed after A ended — if B started while A was still active,
        // transitionMs would be negative, causing false mobile-following upgrades.
        if (firstAtB <= lastAtA) return null
