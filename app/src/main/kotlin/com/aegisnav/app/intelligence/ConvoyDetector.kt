...
class ConvoyDetector @Inject constructor() {
     ...
     *
     * **GPS speed source:** The velocity vector speed is sourced from [userSpeedMps] (the GPS
     * speed of the user's device) rather than being recalculated from haversine distance / time.
     * This ensures convoy speed matches the speed displayed on the map gauge.
     *
        ...
        if (prev != null) {
            ...
            if (dtMs in 1..MAX_VECTOR_INTERVAL_MS) {
                ...
                if (dist >= MIN_VECTOR_DIST_M) {
                    ...
                    // Speed comes from GPS (userSpeedMps), not haversine distance / time,
                    // so it matches the map speed gauge exactly.
                    val speed = userSpeedMps.toDouble()
